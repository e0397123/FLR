...
def main(args, prompt_text, model, tokenizer):
    ...
    if requires_preprocessing:
        ...
        encoded_prompt = tokenizer.encode(prefix + prompt_text, add_special_tokens=False, return_tensors="pt", truncation=True, max_length=512)
    ...
    for generated_sequence_idx, generated_sequence in enumerate(output_sequences):
        ...
        generated_sequences.append(total_sequence.strip())
...
if __name__ == "__main__":
    ...
    parser.add_argument("--prompt_source", type=str, default="")
    ...
    #### LLaMA Template
    prompt_source = json.load(open(args.prompt_source))
    
    new_prompt_source = []
    for d in prompt_source:
        messages = []
        messages.append({'role': 'system', 'content': "You are a helpful assistant."})
        for idx, utt in enumerate(d[:-1]):
            if idx % 2 == 0:
                messages.append({'role': 'user', 'content': utt['value']})
            else:
                messages.append({'role': 'assistant', 'content': utt['value']})
        new_prompt_source.append(tokenizer.apply_chat_template(messages, tokenize=False, add_generation_prompt=True))
            
    prompt_source =  new_prompt_source[args.start_index:args.end_index]
    print(prompt_source[0])
    full_outputs = []
    with open(args.output_path, 'w') as outfile:
        for prompt_text in tqdm(prompt_source):
            rs = main(args, prompt_text, model, tokenizer)
            full_outputs.append(rs)
