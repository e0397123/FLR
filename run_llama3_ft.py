...
if __name__ == "__main__":
    ...
    prefix_sent = ["Please ask me a question and provide feedback to my answer."]
    data = json.load(open(args.input_fname))
    if type(data[0]) is list:
        prompts = [{'conversation': prefix_sent + [d[-2], d[-1]], 'full_conversation': d} for d in data]
    else:
        new_data = []
        for item in data:
            temp = {}
            for k, v in item.items():
                if k == 'conversation':
                    temp[k] = prefix_sent + [v[-2], v[-1]]
                    # temp[k] = prefix_sent + v
                else:
                    temp[k] = v
            temp["full_conversation"] = v
